...
def get_smallest_dir_size(input, total=70000000, target=30000000):
  directories = parse_input(input)
  free_space = total - directories['/'].get_size()
  required_space = target - free_space
  sizes = []
  for _, dir in directories.items():
    sizes.append(dir.get_size())
  sizes.sort()
  for sz in sizes:
    if sz >= required_space:
      return sz
